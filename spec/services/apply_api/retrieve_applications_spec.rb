    ...
    describe "#call" do
      context "when the response is success" do
        ...
        context "when there are multiple pages of results" do
          let(:http_response_page_1) {
            {
              status: status,
              body: ApiStubs::ApplyApi.applications_page_1,
              headers: {
                "Current-Page" => "1",
                "Page-Items" => "2",
                "Total-Pages" => "2",
          ...
          before do
            stub_request(:get, request_url).to_return(http_response_page_1)
            stub_request(:get, request_url_page_2).to_return(http_response_page_2)
          end
